...
import { getCampaign, getProfile, listProfiles } from "../graphql/queries";
...
export default function CampaignUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = campaignRecord
      ? { ...initialValues, ...campaignRecord, profileID }
      : initialValues;
    setName(cleanValues.name);
    setStartsAt(cleanValues.startsAt);
    setFinishesAt(cleanValues.finishesAt);
    setValidFrom(cleanValues.validFrom);
    setValidUntil(cleanValues.validUntil);
    setMinValue(cleanValues.minValue);
    setBonus(cleanValues.bonus);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : campaignModelProp;
      const profileIDRecord = record ? record.profileID : undefined;
      const profileRecord = profileIDRecord
        ? (
            await API.graphql({
              query: getProfile.replaceAll("__typename", ""),
              variables: { id: profileIDRecord },
            })
          )?.data?.getProfile
        : undefined;
      setProfileID(profileIDRecord);
      setSelectedProfileIDRecords([profileRecord]);
      setCampaignRecord(record);
    };
    queryData();
  }, [idProp, campaignModelProp]);
  React.useEffect(resetStateValues, [campaignRecord, profileID]);
