...
import { getProfile, listCampaigns, listSettings } from "../graphql/queries";
import { updateCampaign, updateProfile } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function ProfileUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setCampaigns(cleanValues.Campaigns ?? []);
  ...
  const [linkedCampaigns, setLinkedCampaigns] = React.useState([]);
  const canUnlinkCampaigns = false;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : profileModelProp;
      const SettingsRecord = record ? await record.Settings : undefined;
      setSettings(SettingsRecord);
      const linkedCampaigns = record?.Campaigns?.items ?? [];
      setLinkedCampaigns(linkedCampaigns);
      setProfileRecord(record);
    };
    queryData();
  }, [idProp, profileModelProp]);
  React.useEffect(resetStateValues, [profileRecord, Settings, linkedCampaigns]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const campaignsToLink = [];
          const campaignsToUnLink = [];
          const campaignsSet = new Set();
          const linkedCampaignsSet = new Set();
          Campaigns.forEach((r) => campaignsSet.add(getIDValue.Campaigns?.(r)));
          linkedCampaigns.forEach((r) =>
            linkedCampaignsSet.add(getIDValue.Campaigns?.(r))
          );
          linkedCampaigns.forEach((r) => {
            if (!campaignsSet.has(getIDValue.Campaigns?.(r))) {
              campaignsToUnLink.push(r);
            }
          });
          Campaigns.forEach((r) => {
            if (!linkedCampaignsSet.has(getIDValue.Campaigns?.(r))) {
              campaignsToLink.push(r);
            }
          });
          campaignsToUnLink.forEach((original) => {
            if (!canUnlinkCampaigns) {
              throw Error(
                `Campaign ${original.id} cannot be unlinked from Profile because profileID is a required field.`
              );
            }
            promises.push(
              API.graphql({
          ...
          campaignsToLink.forEach((original) => {
            promises.push(
              API.graphql({
                query: updateCampaign.replaceAll("__typename", ""),
                variables: {
          ...
          const modelFieldsToSave = {
            ...
            profileSettingsId: modelFields?.Settings?.id ?? null,
          };
          promises.push(
            API.graphql({
    ...
    >
      ...
      >
        {profileRecord && (
          <StorageManager
            defaultFiles={[{ key: profileRecord.logo }]}
            onUploadSuccess={({ key }) => {
              setLogo((prev) => {
                let value = key;
                if (onChange) {
